...
Customer Call
     ...
     ▼
┌─────────────────────────────────────────────┐
│         Agent CS Desktop (Angular)          │
│  Dashboard │ Copilot Chat │ Vehicle Details │
│  Warranty  │  Scheduling  │  Vehicle Stats  │
└──────────────────┬──────────────────────────┘
                   │ HTTP/REST
                   ▼
┌─────────────────────────────────────────────┐
...
│  │ (Intent     │   │  Vehicle / Warranty  │ │
│  │  Classify)  │   │  Scheduler/Telemetry │ │
│  └─────────────┘   └──────────────────────┘ │
│         │                    │               │
│  ┌──────▼──────┐   ┌────────▼─────────────┐ │
│  │  RAG System │   │   Services Layer      │ │
│  │  ChromaDB   │   │  Warranty / Scheduler │ │
│  │  KnowledgeB │   │  Telematics Engine    │ │
│  └─────────────┘   └──────────────────────┘ │
│         │                    │               │
│  ┌──────▼────────────────────▼─────────────┐ │
│  │            MySQL Database               │ │
│  │  Vehicles · Customers · Warranty        │ │
│  │  FaultCodes · Appointments · Sessions   │ │
│  └─────────────────────────────────────────┘ │
└─────────────────────────────────────────────┘
         │                    │
         ▼                    ▼
   Google Gemini        Anthropic Claude
   (Intent Class.       (RAG Synthesis,
    General Q&A)         Structured Response)
```
...
| RAG Agent | ❌ Broken | 10% — import error |
| Dashboard warning image analysis | ❌ Not started | 0% |
| Driver notifications | ❌ Not started | 0% |
...
| Resource booking persistence | ❌ Not started | 0% |

**Overall Completion: ~50%**

---

## 11. Pending Features & Roadmap

### P0 — Critical Fixes (Must Fix Before Demo)

| # | Task | File | Issue |
|---|---|---|---|
| 1 | Fix RAG agent import | `agent/rag_agent.py:4` | Wrong import path causes runtime crash |
| 2 | Connect warranty agent → warranty engine | `agent/warranty_agent.py` | Hardcoded for VH002 only |
| 3 | Connect scheduler agent → scheduler service | `agent/scheduler_agent.py` | Returns placeholder text |
| 4 | Connect telemetry agent → telematics service | `agent/telemetry_agent.py` | Returns placeholder text |
| 5 | Hash passwords | `api/user.py:39` | Plaintext storage is a security vulnerability |

### P1 — Core Feature Gaps

| # | Feature | Description |
|---|---|---|
| 6 | Expand knowledge base | Add service manuals, OBD-II reference, TSBs to `knowledge_base/` |
| 7 | Wire RAG into orchestrator | Call RAG agent as fallback when no structured agent answers |
| 8 | Dashboard warning image analysis | Use Gemini Vision to identify warning lights from uploaded photos |
| 9 | Scheduling API → Frontend | Connect Angular scheduling form to `POST /api/scheduling/slots` |
| 10 | Booking persistence | Save confirmed appointments to `service_appointments` table |

### P2 — Missing Capabilities

| # | Feature | Description |
|---|---|---|
| 11 | Driver notification system | Email/SMS alerts for service reminders and appointment confirmations |
| 12 | Expand DTC database | Current: 6 codes. Target: Full SAE J2012 standard codes (~4,000+) |
| 13 | Predictive maintenance | ML-based part failure prediction from telematics time-series |
| 14 | Salesforce integration | CRM sync for case creation and vehicle lookup from real OEM DB |
| 15 | Real-time OBD-II feed | WebSocket connection for live vehicle sensor streaming |
| 16 | Reschedule / cancel appointments | Appointment lifecycle management |
| 17 | Role-based access | Agent vs supervisor vs admin permissions |
| 18 | Refresh token | JWT refresh token flow (currently single token, no expiry refresh) |
...
*Generated for Automotive CS03 · AI Automotive Customer Service Copilot · March 2026*
